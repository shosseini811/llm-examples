#!/usr/bin/env python3
# BPE (Byte Pair Encoding) Implementation from Scratch
# This implementation demonstrates the core BPE algorithm used in modern LLMs
...
class BPETokenizer:
    """A complete implementation of the Byte Pair Encoding algorithm.
    
    BPE is a subword tokenization algorithm that iteratively merges the most frequent
    pairs of adjacent tokens (starting with characters) to form new tokens.
    
    This implementation includes:
    1. Training a BPE model from a corpus
    2. Encoding text using the trained model
    ...
    def _get_stats(self, sequences: List[List[str]]) -> Dict[Tuple[str, str], int]:
        """Count frequency of adjacent token pairs in all sequences.
        
        Args:
            sequences: List of tokenized sequences (each a list of strings)
    ...
    def _merge_pair(self, sequences: List[List[str]], pair: Tuple[str, str], 
        ...
        """Merge all occurrences of a specific pair in all sequences.
        ...
        Returns:
            Updated sequences with the pair merged
        """
        first, second = pair
        new_sequences = []
    ...
    def save(self, path: str) -> None:
        ...
        for k, v in self.merges.items():
            if isinstance(k, tuple) and len(k) == 2 and k[0] and k[1]:
                valid_merges[f"{k[0]} {k[1]}"] = v
            else:
                print(f"Warning: Skipping invalid merge rule: {k} -> {v}")
...
def generate_sample_corpus() -> List[str]:
    ...
    Returns:
        List of text documents/sentences
    """
    return [
